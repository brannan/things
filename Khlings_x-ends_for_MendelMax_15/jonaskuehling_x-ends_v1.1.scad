...
wall = 4.5;			// default wall thickness 3
//wall_thin = 3;
xend_body_length = 40;
	...
	difference(){
		...
		if(idler==false){
			...
			translate([-nema17_hole_dist/2-2,0,nema17_hole_dist/2+m3_screw_dia/2+clearance+xend_body_height]){
				...
				// PSB Add 1
				translate([nema17_hole_dist/2 +3.5,0,-nema17_hole_dist/2 + 2.3]) 
					cube([4,body_width+3+2*wall+2,8],center=true); // added cube to cut out junk
					
				translate([nema17_hole_dist/2,0,-nema17_hole_dist/2 ]) {
					rotate([0,45,0])
						translate([-2,0,4 + 2.5]) // PSB Hack + 2 and stretched cube 5 z below
							cube([4,body_width+3+2*wall+2 ,8 + 5],center=true); // 45 deg cut in motor support
				}
				translate([-nema17_hole_dist/2,0,nema17_hole_dist/2]){
