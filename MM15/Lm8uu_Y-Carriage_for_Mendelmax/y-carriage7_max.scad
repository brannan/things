...
three_lm8uu=0;                       // How many lm8uus to use: 0 = 4 lm8uu, 1 = 3 lm8uu.
//single_lm8uu_side=1;                // Single bearing side selection: -1 = left, 1 = right.

three_legged=0;                      // three or four legged: 1 = three, 0 = four. 
...
module spacer()
{
  difference() 
  {
	...
	for (i=[-1,0]) {
		...
		translate(v=[lm8uu_holder_width*3/4+i*(lm8uu_holder_width*6/4), 0, 10])
		cylinder(r=7.05/2, h=extension_block_thickness+2, $fn=10);
	}
	...
	translate([0, 0, extension_block_thickness]) rotate([0,0,30]){
		#nut(d=m3_nut_diameter, h=2.5);
